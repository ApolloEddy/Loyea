 ...
 *
 * 语义对齐 SillyTavern World Info v2（按 Loyea Rebuild Spec §6 调整）：
 * - 关键词触发 / constant 常驻 / selective 次级关键词四种逻辑（AND_ANY / NOT_ALL / NOT_ANY / AND_ALL）
 * - 扫描窗口只由 scan_depth（条目覆盖或全局）决定；entry.depth 仅作插入深度语义
 ...
 * - 预算按整条取舍、每轮递归后重算；被预算排除的条目内容不再触发递归，
 *   常驻条目超预算时给出 constantOverflow 可恢复错误信号
 * - 分组邻接 / 排序模式 / token 预算裁剪；trace 区分入选/递归/预算排除/未匹配
...
object WorldInfoMatcher {
    ...
    /** 单条目的预算取舍记录（整条取舍，不截断半条，Spec §6.2.5）。 */
    data class BudgetExclusion(val entryId: String, val reason: String) {
        companion object {
            const val REASON_CONSTANT_OVERFLOW = "constant_overflow"
            const val REASON_BUDGET_EXHAUSTED = "budget_exhausted"
        }
    }

    /** 匹配过程 trace：区分入选、递归激活、被预算排除与未匹配。 */
    data class WorldInfoTrace(
        val selectedIds: List<String> = emptyList(),
        val recursionActivatedIds: Set<String> = emptySet(),
        val budgetExcluded: List<BudgetExclusion> = emptyList(),
        val unmatchedIds: List<String> = emptyList(),
        /** 常驻条目本身放不进预算：可恢复错误，宿主应暂停发送并提示（Spec §6.2.6）。 */
        val constantOverflow: Boolean = false
    )

    data class WorldInfoMatchResult(
        val entries: List<WorldInfoEntry>,
        val runtimeState: WorldInfoRuntimeState,
        val trace: WorldInfoTrace = WorldInfoTrace()
    )
    ...
    fun matchWorldInfoEntriesFor(
        ...
        val budgetExcluded = ArrayList<BudgetExclusion>()
        val excludedIds = linkedSetOf<String>()
        val recursionActivated = linkedSetOf<String>()
        var constantOverflow = false

        // Spec §6.2.5/6.2.7：预算按整条取舍；被预算排除的条目不再作为递归来源，
        // 也不会在本请求内被重复选中。常驻条目放不进预算时记录 constantOverflow。
        fun pruneOverBudget() {
            val limit = when {
                config.tokenBudget <= 0L && config.budgetCap <= 0L -> return
            ...
            val orderedForBudget = matched.values.sortedWith(
            ...
            for (candidate in orderedForBudget) {
                ...
                if (cost <= 0L) {
                    excludedIds.add(candidate.id) // 空内容不注入也不占预算
                    continue
                }
                if (used + cost <= limit) {
                    keep.add(candidate.id)
                    ...
                    val isConstant = candidate.constant
                    if (isConstant) constantOverflow = true
                    budgetExcluded += BudgetExclusion(
                        candidate.id,
                        if (isConstant) BudgetExclusion.REASON_CONSTANT_OVERFLOW
                        else BudgetExclusion.REASON_BUDGET_EXHAUSTED
                    )
                }
            }
            matched.keys.retainAll(keep)
        }
        ...
        // 递归来源只取预算内条目（Spec §6.2.7：预算排除的内容不产生因果链）。
        if (config.allowRecursion && config.recursionDepthCap > 0) {
            ...
            while (pass <= config.recursionDepthCap && !prevent) {
                // 递归来源：仅 allowRecursion 且在预算内的条目 content 参与扫描
                ...
                for (e in active) {
                    if (e.id in matched || e.id in excludedIds) continue
                    tryActivate(e, worldContent, pass)?.let { candidates += it }
                }
                val added = resolveCandidates(candidates)
                added.forEach { recursionActivated += it.id }
                pruneOverBudget()
                if (added.any { it.preventRecursion }) prevent = true
                // 仅当本轮无新增「且」无仍在等待延迟激活的候选时才提前终止：
                // 否则 delayUntilRecursion 条目（其 turn 在后续轮次）会被错误地跳过
                val pendingDelay = active.any { it.id !in matched && it.id !in excludedIds && it.delayUntilRecursion > pass }
                if (added.isEmpty() && !pendingDelay) break
                pass++
            }
        }

        if (matched.isEmpty()) return WorldInfoMatchResult(
            emptyList(),
            nextRuntimeState,
            WorldInfoTrace(
                selectedIds = emptyList(),
                recursionActivatedIds = recursionActivated,
                budgetExcluded = budgetExcluded,
                unmatchedIds = active.map { it.id }.filter { it !in excludedIds },
                constantOverflow = constantOverflow
            )
        )

        // —— 排序 + 分组邻接（同组保持连续，不被他组/无组条目打断）——
        // 预算取舍已在每轮 pruneOverBudget 中完成（常驻优先、priority/order 次之），
        // 这里只负责按配置顺序渲染。
        val selectedSorted = matched.values.sortedWith(entryComparator(config))
        ...
        return WorldInfoMatchResult(
            ordered,
            nextRuntimeState,
            WorldInfoTrace(
                selectedIds = ordered.map { it.id },
                recursionActivatedIds = recursionActivated,
                budgetExcluded = budgetExcluded,
                unmatchedIds = active.map { it.id }.filter { it !in matched && it !in excludedIds },
                constantOverflow = constantOverflow
            )
        )
    }
    ...
    fun isActivated(
        ...
        val sourceTexts = buildString {
            ...
            // ST 递归语义（Spec §6.2.2）：递归轮把已命中条目的正文并入扫描缓冲，
            // 不要求条目显式声明 world 源；显式 SOURCE_WORLD 则在直接轮也可扫描 world。
            if (SOURCE_WORLD in sources || worldContent.isNotEmpty()) append("\n").append(worldContent)
        ...
        // W02：secondary 全空时 selective 不生效，退化为普通关键词匹配。
        if (entry.selective && !entry.constant && entry.keysecondary.any { it.isNotBlank() }) {
    ...
    private fun activationScore(
        ...
        val sourceTexts = buildString {
            ...
            if (SOURCE_USER in sources) append("\n").append(userName)
            if (SOURCE_SYSTEM in sources) append("\n").append(systemPrompt)
            // 与 isActivated 一致：递归轮的 world 内容参与评分（Spec §6.2.2）
            if (SOURCE_WORLD in sources || worldContent.isNotEmpty()) append("\n").append(worldContent)
            if (entry.matchPersonaDescription) append("\n").append(personaDescription)
            if (entry.matchCharacterDescription) append("\n").append(characterDescription)
            if (entry.matchCharacterPersonality) append("\n").append(characterPersonality)
    ...
    // Spec §6.1：扫描深度只由 scan_depth 决定（条目显式覆盖或全局配置）；
    // entry.depth / extensions.depth 是插入深度语义，不得当作扫描窗口误读。
    private fun effectiveDepth(entry: WorldInfoEntry, config: WorldInfoConfig): Int =
        entry.scanDepthOverride?.takeIf { it > 0 } ?: config.scanDepth
