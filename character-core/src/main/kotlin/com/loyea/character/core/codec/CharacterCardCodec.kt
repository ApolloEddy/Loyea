...
data class CardData(
    ...
    val characterBook: CardBook? = null
)
...
data class CardBook(
    ...
    val extensionsJson: String = "{}",
    val entries: List<CardBookEntry> = emptyList(),
    val rawJson: String? = null
)

data class CardBookEntry(
    val id: Int? = null,
...
object CharacterCardCodec {
    ...
    fun parseJson(json: String): CardDocument? {
    ...
    fun parseCharacterBook(json: String): CardBook? =
     ...
     */
    fun parseCharx(input: InputStream): CardDocument? {
        return try {
     ...
     */
    fun parseCharxWithAssets(input: InputStream): CharxArchive? {
        return try {
            ZipInputStream(input).use { zip ->
                var entryCount = 0
                var totalBytes = 0
                ...
                CharxArchive(document, selected)
            }
        } catch (_: Exception) {
            null
        }
    ...
    fun parsePng(input: InputStream): CardDocument? {
        return try {
    ...
    fun toJson(document: CardDocument, targetSpec: String = document.spec): String {
    ...
    fun stableId(document: CardDocument): String {
        val identity = listOf(
            document.data.name,
    ...
    private fun parseData(data: JsonObject): CardData {
    ...
    private fun parseBook(book: JsonObject): CardBook {
    ...
    private fun parseEntry(entry: JsonObject, fallbackId: Int): CardBookEntry {
    ...
    private fun writeData(data: JsonObject, value: CardData, targetSpec: String) {
    ...
    private fun bookToJson(book: CardBook): JsonObject {
    ...
    private fun entryToJson(entry: CardBookEntry): JsonObject {
    ...
    private fun decodeCandidate(value: String): CardDocument? {
    ...
    private fun referencedCharxAssetNames(document: CardDocument): Set<String> {
