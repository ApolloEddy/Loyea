...
import com.loyea.character.core.worldinfo.*

/**
 * 将角色卡内嵌 CharacterBook 映射为 Loyea 的运行时世界书。
 *
 * 适配器不修改原始 JSON；所有 ST/V3 扩展仍留在 CardBook.extensionsJson
 * 和 entry rawJson 中，运行时只把当前引擎需要的字段投影出来。
 */
object CardBookAdapter {
    fun toWorldInfoBook(book: CardBook, idPrefix: String): WorldInfoBook {
