...
1. **真机对照**：NFR-06/07 剩余真机部分（发热/温控/厂商电源策略）需提供真机与数据线，或接受模拟器对照数据作为本期验收上限。
2. **版本号**：是否升级 versionCode/versionName 并发布（当前 0.8.2/20 未动）。
3. **推送**：`plugin/companion` 分支是否推送远端（当前仅本地提交）。
