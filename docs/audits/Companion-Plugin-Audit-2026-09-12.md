...
> 2026-09-13 六次更新（验收修复）：独立审查报告 docs/Loyea_Companion_Review_af97430.md 判定本轮不通过完整验收并列出 R-01~R-09，全部问题已在 integration/plugins 修复。要点：R-01 感知开关经会话 useSystemTime 统一生效（创建/绑定/改开关三处同步）；R-02 陪伴固定记忆改为自动整理只读，普通会话修订号条件提交+空结果 no-op+整理水位——本报告旧版「自动整理仅写图谱」「固定记忆保护已生效」的表述作废，以本更新为准；R-03 语音转写任务归属当前会话、切换/停止即取消；R-04 存储写入成败上报 + 恢复事务化（staging→原子提交→清理，失败保留旧数据）；R-05 备份 v2 补全配置与图谱记忆并重映射、重新开始清图谱；R-06 主动联系完整门控合同（生成前+提交前双执行、台账限频、稳定事件 ID、通知路由回陪伴会话）；R-07 消息文件缺失/损坏进入恢复态并可从备份恢复；R-08 陪伴称呼参与实际 Prompt；R-09 录音状态源修正、搜索后台线程化、回到底部/图片预览/音频入口/头像/聊天服务模式内切换。单测 app 202 项 + modulator 35 项全绿。
