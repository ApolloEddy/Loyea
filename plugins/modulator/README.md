...
web 页自测：`node plugins/modulator/web/selftest.cjs`（fixture 正确性 / 请求串行化 / 重置竞态 3 组正确性断言）。

## 已知语义边界（Spec v1.0 既有行为，变更需规格裁定）

GPT 反例复现包（`docs/loyea_modulator/validation/gpt_counterexamples_7f189e9/`，逐条处置见其 `RESPONSE.md`）确认两点模型语义与参考实现逐位一致，属规格设计边界而非移植缺陷：

1. **痕迹按 `(kind, target, topic_id)` 合并**：同话题同指向的两条同类威胁会合入一条痕迹，`threat_resolved` 只关联其一也会按 §8.3 公式衰减整条痕迹。
2. **可见性以痕迹为单位**（§9.3）：合并痕迹的总量会随任一可见证据呈现，隐藏强证据可经可见弱证据"泄漏"显示。

两者若要按证据粒度归因/衰减，需细化痕迹键并重生成 golden 契约（Spec v1.1 议题）。
