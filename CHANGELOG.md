...
> 陪伴模式独立验收修复（R-01~R-09，对照 docs/Loyea_Companion_Review_af97430.md）：修复后陪伴模式
> 随 integration/plugins 并入 main 正式纳入；调制器插件（plugin/modulator）同批并入。
> 其后两批新能力：**陪伴智能接入**（情绪感知 ➜ 调制 ➜ 私有 Lore ➜ 真实请求闭环）与
> **伴读插件**（plugin/reader，读屏伴读 + 防剧透问答）。

### Added (新增)
- **陪伴智能接入（Spec §13.1 P0–P6）**：陪伴会话的每条消息接入端侧智能管线——文本情绪感知模型
  （onnxruntime，端侧一次推理 500ms 截止、三连败熔断，词片/解码/校准与原实现逐 logits 黄金一致）➜
  调制器证据归因 ➜ 状态投影 + 8 条版本化私有 Lore，以固定标记块进入实际出站请求；SQLite 运行账本
  （CAS 事务/observation 去重/outbox），编辑已发消息沿检查点回放新分支；工具失败记独立观测；
  文字感知独立开关；备份 v3 含运行账本，恢复/重开换代围栏；问候读同一状态不重复感知；
  陪伴设置新增内部状态可视化页（轴向量/证据/消解关系，纯观测）。
- **调制器 v1.1.0 证据归因修订**：聚合强度改为最多 4 条证据级组件（各带独立衰减），消解只抑制
  显式链接的证据组件，状态对外只暴露纯投影 API；v1.0.0-prototype 旧检查点自动迁移（归因重置不崩）。
- **伴读插件（plugin/reader）**：无障碍读屏伴读——白名单采样（默认 Chrome，密码框跳过）➜ 正文提纯
  （广告/进度条/样板剥离、章节识别）➜ 防剧透游标物理截断上下文 ➜ 滚动摘要 + 实体卡 ➜
  悬浮球（NeuralLiving 迷你画布 15fps）对话面板向当前对话渠道提问，回答基于已读内容；
  防剧透开启时涉及未读的提问本地拒绝不发起网络；正文只在内存、服务销毁即清空、不进备份。
- **主动联系完整合同（R-06）**：门控（通知权限/每日 2 条/≥4h 间隔/30 分钟静默/上次未回应抑制/前台不打扰/免打扰跨午夜）在生成前与提交前双执行；台账持久化限频与稳定事件 ID（崩溃重投不重复投递）；通知点击路由回陪伴会话；普通模式问候路径原语义保留、绝不以陪伴会话为目标。
- **备份 v2 与事务化恢复（R-04/05）**：备份补全陪伴设置（称呼/免打扰/开关）与图谱记忆三元组；恢复 staging→原子换列表→清旧，任一步失败保留旧数据并明确报错；重新开始清聊天/固定/图谱记忆/草稿；恢复态（FUN-09）新增「从备份恢复」入口。
- **感知开关统一生效（R-01）**：陪伴感知总开关经会话 useSystemTime 载体，创建/绑定/改开关三处同步；前台上下文、感知工具执行、记忆敏感过滤、后台问候同一状态源，普通模式开关独立。
- **陪伴称呼接入 Prompt（R-08）**：陪伴会话请求与后台问候使用「希望被称呼的名字」，普通模式维持宿主用户名。
- **交互补齐（R-09）**：回到底部/未读指示、图片全屏预览、音频气泡播放入口、头像更换（本地持久化）、聊天服务模式内快速切换、神经元舞台小屏自适应。

### Changed (变更)
- **固定记忆保护重写（R-02）**：陪伴固定记忆为用户独占（自动整理不再调用模型改写）；普通会话核心记忆提交改为修订号条件合同——用户中途增删改即整轮作废、空结果严格 no-op 不清空、★ 锁定事实强制保留置顶；新增整理水位，每条消息至多参与一次整理，图谱 mentionCount 不再因滑窗重叠虚高。
- **存储层可靠性（R-04/R-07）**：所有会话写入返回成败；会话不存在时 updateSessionMessages 拒写（防复活护栏，迟到的问候/图注/快照写回无法重建已删会话）；消息文件探测区分合法空/缺失/损坏，绑定前校验，损坏历史不再被当成空会话；原子写以 Files.move(REPLACE_EXISTING) 替代非原子直接覆盖回退。
- **语音任务归属（R-03）**：转写任务捕获来源会话，切换会话/恢复/重开/关闭陪伴显式取消并在回发前校验归属；录音取消竞态（200ms 避让窗口）与快按快放竞态修复；录音状态以 ViewModel 真实状态为唯一 UI 来源（权限被拒不再显示"正在聆听"）。

### Fixed (修复)
- **伴读面板跨线程崩溃**：LLM 回调在 IO 线程直接触碰主线程创建的面板 TextView（CalledFromWrongThreadException 闪退）；现统一切回主线程追加会话行。
- **空 Key 配置伪装可用**：对话渠道激活配置缺 API Key 时仍被解析为可用，请求必然 401 且用户只见「服务出错」；现解析层直接判未配置并提示补全，附回归单测。
- **恢复态"重新开始"死循环**：绑定记录存在但消息文件缺失/损坏时，仅清绑定声明会让 bind 重新发现坏条目再次进入恢复态；现先移除坏条目（仅元数据，原数据文件保留）再重建。
- **写失败不清空时间线**：updateSessionMessages 被护栏拒绝或写盘失败时，消息更新调用方不得把内存消息列表替换为空（合并/版本切换/编辑四处收敛）。
- **主动问候身份校验**：生成期间发生恢复换绑/重开重建时，提交前比对会话 id 与 bindingRevision，不一致即丢弃，旧历史生成的问候绝不落入新会话。
- **通知门控与路由**：SDK<33 的系统级通知屏蔽纳入门控（屏蔽即不生成不落盘）；旧问候通知不得静默重开已关闭的陪伴模式。
- **数据操作撤销整理任务**：恢复/重开前撤销进行中的记忆整理 WorkManager 任务，旧图谱不再被迟到的写回复活；感知开关同步失败时不回写内存；新会话空消息文件落盘失败时回滚创建。

## [0.8.2] - 2026-09-09

> 0.8.1 发布后的增量版本：对话静默失败全量透出、渠道不支持流式自动降级闭环、非标准响应体容错。三次实机排障沉淀（小马 New API 中转渠道）。

### Added (新增)
- **渠道不支持流式自动降级**：检测到 HTTP 200 + "不支持流式"类错误体（或空响应/非 SSE 杂体）时，自动改用整包（非流式）请求重试一次，回复、思考、工具调用照常入会话；降级事实以 Toast 反馈（反馈式修复优于静默放宽）。
- **非流式快速路径**：渠道首次自动降级后按端点+模型记录，本配置后续消息跳过注定失败的流式往返直接整包请求，回复延迟减半；仅内存态，应用重启后自愈重试流式。
- **StreamEvent.Notice 非致命提示事件**：不打断回复流的用户告知通道（标题/收尾/后台问候路径静默忽略）。

### Fixed (修复)
- **静默空回复五连堵**：HTTP 200 但响应体不是 SSE（网关 HTML/错误 JSON/空流）不再伪装成正常结束——按空响应/服务商标错原文/整包 JSON 降级解析/不可识别体（附采样）四分类透出；流结束零内容零工具透出"服务商返回了空回复"；finish_reason=length/content_filter 透出"回复因输出上限被截断"；SSE data: 无空格序列化兼容（部分网关省略冒号后空格，旧实现整行丢弃）；后台任务非流式路径零内容同规则堵死。
- **非标准响应体容错**：顶层裸 JSON 字符串、choices[0].message 为字符串两类网关变体直接按正文解析（小马渠道实测形态，旧实现 Gson 报 Expected JsonObject but was JsonPrimitive 致降级重试也失败）；解析失败时错误文本附响应体开头采样 + logcat 留档，现场不再依赖复现定位。

## [0.8.1] - 2026-09-08

> 0.8.0 发布后的增量版本：识图链路五连修、打开会话落位末尾、语音转写截断修复与 Key 加密漏改收尾。

### Added (新增)
- **智谱服务商预设**：服务商列表新增 Zhipu (智谱)（open.bigmodel.cn/api/paas/v4，默认模型 glm-5.3-flash），智谱用户不再被迫走 Custom 配置而被视觉能力判断误伤。
- **识图诊断日志**：新增 LoyeaVision 诊断日志（路由决策/请求端点与模型/图片 base64 长度/解码失败原因），实机排障不再靠猜。
- **视觉通路契约测试**：带图消息进入请求 payload 的三条不变式锁定为回归测试。

### Changed (变更)
- **版本号升级至 v0.8.1**：内部 `versionCode` 同步升级至 19，可覆盖安装 v0.5.5 及之后的任意版本（签名一致）。
- **打开会话落位到会话末尾**：取代此前"锚定最后一条用户消息"的载入定位——长回复（思考条/工具卡/语音）比一屏高时结尾落在折叠线以下，被感知为"打开不是最后一条对话"；现打开应用/切换会话直接定位到最新消息（初始渲染窗口改为尾部 30 条，滚动仍等待数据提交后执行；落位后恢复流式跟随，触摸列表即交还控制权）。
- **应用图标换新**：桌面自适应图标与侧栏/收起态顶栏头像统一换用新版图标（新增 ic_avatar 圆形头像资源，各密度重绘）。

### Fixed (修复)
- **语音转写断半句**：AI 语音消息的转写文字在文本含英文引号处被拦腰截断、尾部残留孤立反斜杠（TTS 音频始终正常——音频走正规 JSON 解析，转写却用惰性正则提取，遇 JSON 转义 `\"` 在第一个转义引号处提前终止）；现以正规 JSON 解析优先、转义感知正则兜底，收敛为共享的 VoiceTextExtractor（同时消除 UI/VM 两份漂移副本与单遍反转义顺序缺陷）；转写为展示期提取，历史消息重开即自动补全。
- **拍照/发图后没有任何回复（识图链路五连修）**：
  - **降级重试从未执行**：视觉请求报错后降级分支只剥图清气泡、循环在无工具分支直接 break，重试从未发生——留下空气泡即"发图无回复"；现降级后真正重发请求（还原主配置/主模型并恢复工具能力），降级动作以 Toast 透出服务商标错原文。
  - **识图模型名覆盖雷**：显式识图卡的模型名被"视觉模型"字段内置默认 `gpt-4o-mini` 覆盖，智谱等自配用户必 400；现字段未自定义时识图卡自带模型名优先，`providerSupportsVision` 补智谱 glm-4.5v 与原生多模态 glm-5.3 系列。
  - **纯图片消息空文本块**：只发图不打字时多模态 content 数组携带 `"text": ""` 空文本块，严格网关判参数非法（智谱 1214 一类）——现无文字时不加 text 块（图片/音频路径同改）。
  - **识图分辨率与 EXIF 方向**：800px 压缩使密排卷面字高仅 4-6px，模型"看得见图读不了字"，升到 1280px/85（实测可完整读题解题）；竖拍照片按 EXIF 方向摆正后再发送。
- **API Key 加密迁移两处漏改**：GreetingWorker 仍读明文配置键（迁移删明文后恒空 → 链式自排调度断链，后台问候静默永久失效）；saveApiConfigList 只写明文不写加密库（写读不同库，改动的 API 配置重启即丢）。现问候/配置统一走加密库唯一通道，散装明文键迁移时清除，问候"无事可做"改为续排不再断链。

## [0.8.0] - 2026-09-07

> v0.7.2 发布后的增量版本：世界书卡书编辑、时间标签方案 A、对话式生图、全量英文化、会话交互升级与全量审计修复。

### Added (新增)
- **对话式生图（generate_image 工具）**：生图 API 配置好后对话中直接说"画一张……"即可创作并展示（此前只有手动 `/draw` 指令可用）；支持 url 与 b64_json 两种响应形态；图片在气泡内以更大画幅完整展示（不再裁切），保留骨架加载与点击查看原图。
- **会话页回到底部气泡**：上滑离开底部时右下角浮现，点击回最新消息并恢复流式跟随；离底期间新消息攒未读角标（99+ 封顶）。
- **会话载入锚定**：打开应用/切换会话时内容定位到用户最后一次发消息处（即时定位、略带上文），不再回到顶部或任意位置。
- **书库条目全文查看**：点条目正文弹出底部弹层（标题/关键词/全文/元信息），弹层内直达编辑。
- **卡书条目编辑**（override 层）：角色卡内嵌书条目内容可改、可"恢复原文"，原卡文件一字节不动。

### Changed (变更)
- **时间标签方案 A**：用户消息逐条携带发送时刻（时间观念完整保留），assistant 历史回复永不带标签；删除全局"禁一切方括号"禁令，改为"默认不输出时间标签（角色设定或用户明确要求时放行）"的窄规则——防泄露与主动要标签不再互斥。
- **全量英文化**：全部界面补齐英文分支（设置/书库/角色页/会话页/组件/通知），术语统一、翻译自然；英文语言下可完整交互。
- **工具卡标题人话化**：内置工具映射为动作描述（双语），搜索/读网页带查询词，外部工具不再显示裸名。
- **中文消息时间七段细分**：凌晨/早上/上午/中午/下午/晚上/深夜。
- **工具轮耗尽收尾轮**：代理循环到顶后先发一次无工具请求让 AI 把话说完，替代直接落系统占位文案。
- 多轮工具响应期间不把上滑回看的用户拽回底部；换会话即重置内容锚点。

### Fixed (修复)
- **时间标签泄露根因**：assistant 历史回复不再携带时间标签（切断模型自我格式模仿），流式半标签问题根治。
- **Thinking 计时冻结**：工具执行间隙秒数不再停走（tick 驱动重组修复）；时钟向未来跳变钳 0；计时逻辑抽为 ThinkingTimer 纯函数并加压力用例。
- **快速切换会话竞态**：晚到的旧会话加载结果不再覆盖新会话（此前可能把旧会话消息永久并入新会话文件）；/draw 生图落盘同样补齐会话守卫。
- **删除会话不再连带删除全局生效的世界书**（会话与会话书解耦，仅解绑保留）。
- **导入卡路径生图文档误嵌**（关搜索+开生图时模型看不见生图能力）；世界书导出 uid 撞号；嵌入内容折叠键插值事故；生图下载半截文件；死卡书抢占全局生效；收尾轮 haptic 泄漏与失败静默；lint 门禁 59 error 清零等——详见审计修复批次。



### Fixed (修复)
- **快速切换会话的竞态**：会话异步加载写回加代际守卫，晚到的旧会话结果不再覆盖新会话（此前可能把旧会话消息永久并入新会话文件）；/draw 生图的占位写入、完成与失败落盘同样补齐会话守卫（落盘改为从磁盘读回目标会话后只更新对应消息，杜绝跨会话写污染）。
- **自动跟随滚动差一项**：非思考阶段自动滚动的目标是倒数第二条消息而非底部（系统性偏差，每次回复结束都偏离）——修正目标下标。
- **工具轮耗尽收尾轮健壮性**：收尾请求期间保持流式态（停止按钮可用）；收尾文本剥离完整 `[haptic:xxx]` 标签防止渲染泄漏；收尾流失败且零产出时向气泡留痕提示；计入收尾请求的 token 用量。
- **生图下载完整性**：图片下载改为临时文件 + 重命名落盘，下载中断不再把半截文件当成功绑定到消息；并发场景文件名以 UUID 后缀规避同毫秒碰撞。
- **导入卡路径生图文档误嵌**：`generate_image` 工具说明被嵌在联网搜索开关块内，"关联网搜索 + 开生图"的导入卡角色完全看不见生图能力——已移出为独立开关，与原生路径行为对齐。
- **世界书导出 uid 撞号**：uid<=0 的条目改用「现有最大 uid 之后」递增编号，不再与真实 uid=1 的条目产生重复 uid。
- **卡书 override 的 disable 归一化**：内容 override 应用时强制 `disable` 跟随卡内原生状态，消除重导同卡后"详情显示启用但运行时不注入"的展示/匹配不一致。
- **死卡书抢占全局生效**：来源卡已删除的卡书不再被设为全局生效或绑定会话（此前会清空其它书的全局标记，且解析层过滤后所有未绑定会话的世界书静默失效）。
- **嵌入内容折叠键插值事故**：代码块折叠状态键的 `${'$'}` 字面量错误使所有折叠条共享同一状态键（点一个全体联动）——修正插值。
- **Thinking 标题双语**：中文模式显示"深度思考中 · Xs / 已深度思考 · Xs"（原为硬编码英文）。
- **杂项**：旧消息无有效时间戳不再向模型注入 "[MESSAGE TIME: unavailable]" 占位行（消除潜在模仿源）；生图响应缺 url 字段（b64_json 形态）或 200 非 JSON 时给出明确失败原因并放行取消异常；GreetingWorker 不再向问候提示词注入自相矛盾的语音工具声明；角色页 "Creator: Unknown" 中文漏翻补齐。

### Added (新增)
- **会话导出 / 导入备份**：会话菜单支持导出 Markdown（可读）与 JSON（可恢复备份）；导入 JSON 生成全新会话（新 id，图片等外部媒体不迁移，保留文本与图注）。
- **图片自动图注**：识图消息后台生成 ≤15 字短描述；文本模型降级路径与后台问候历史以 `[图片｜描述]` 引用（纯文本模型也能理解图片语境）。

### Changed (变更)
- **流式热路径性能**：消息列表改为 SnapshotStateList，流式文字块 O(1) 原地更新（此前每个文字块全表复制，长会话卡顿根源）。
- **API Key 加密存储**：服务商配置迁入 EncryptedSharedPreferences（AES256，密钥托管 Keystore），明文一次性迁移并移除。
- **媒体缓存统一管理**：设置 → 多模态新增缓存卡片，自动清理时长可选（关闭/1/3/7/30 天，默认 7 天）+ 手动立即清理；原 TTS 三天专项策略并入；消息引用中的图片受保护。

### Fixed (修复)
- **回到底部角标 99+ 异常**：计数改为按「最新消息是否在屏幕上」判定（锚定阅读位不再累积假未读），最新消息可见即清零。
- **会话载入/切换锚点时序竞态**：snapshotFlow 等待消息到达后定位（确定性），锚定期间自动滚动让位。
- **物理感知泄露绕过 ×2**：XML 文本兜底调用不经过工具列表过滤——wifi/噪音授权与联网开关现已统一拦截。
- **API Key 加密迁移的两处漏改**：GreetingWorker 仍读明文键（迁移删明文后永远为空 → 链式自排调度断链，后台问候静默永久失效）；saveApiConfigList 只写明文不写加密库（写读不同库，改动的 API 配置重启即丢）且 selectActiveConfig 散装明文键（api_key 等，无读方）残留泄露面。现问候/记忆提炼统一走加密库，配置唯一落库通道 = 加密库，散装键迁移时清除，问候"无事可做"改为续排不再断链。
- **深度思考吸顶条**：思考/工具期间常驻顶部的胶囊（实时计时），随时点击折叠/展开。
- **带图请求失败静默留空泡（降级重试从未执行）**：视觉路由请求报错后，降级分支只剥图占位、清空气泡，循环却在无工具分支直接 break——重试从未发生，留下内容为空且非错误的气泡，表现为"拍照/发图后没有任何回复"；下一条纯文本消息的历史图片又只剩 `[图片]` 占位，模型只能回答"看不到图"。现在降级后真正重发请求：还原主配置/主模型并恢复工具能力（失败原因若是识图模型本身不可用，重试才有机会成功），降级动作以 Toast 向用户可见（反馈式修复优于静默降级）；另堵上图片文件读取/解码失败时发送空 base64 畸形 data URL（服务端必 400）的地雷，改为占位文本照常发送。
- **识图模型名覆盖雷 + 智谱能力识别缺失**：显式识图卡选中后，请求模型名一律被"视觉模型"字段的内置默认值 `gpt-4o-mini` 覆盖——选了卡但从未改过该字段的用户（智谱自配 Custom 为典型），识图请求打向服务商不存在的模型必 400；现改为：字段未自定义时识图卡自带模型名优先，显式改过仍以显式值为准。`providerSupportsVision` 智谱分支补 `glm-4.5v`（含 "4.5v" 而非 "4v"，旧列表永不匹配）与原生多模态的 `glm-5.3` 系列；服务商预设列表新增 Zhipu (智谱)（open.bigmodel.cn/api/paas/v4，默认模型 glm-5.3-flash），智谱用户不再被迫走 Custom 而被视觉能力判断误伤。
- **纯图片消息空文本块 + 降级报错不透出**：只发图不打字时多模态 content 数组携带 `"text": ""` 空文本块，严格网关会判结构非法（智谱 1214 一类"参数非法"）——现无文字时不加 text 块（图片/音频路径同改）；多模态降级 Toast 原先只说"已降级"不说原因，服务商标错原话被吞进静默空气泡无从排查——现 Toast 携带报错原文（截断 120 字）并落 Log.e。
- **识图分辨率与 EXIF 方向**：图片上传上限 800px + 质量 80 会把整页密排文字压到 4-6px 高——模型"看得见图读不了字"（行测卷面"只见图不见内容"成因之一），升到 1280px/85（实测同页 109KB→295KB，模型可完整读题解题）；BitmapFactory 不应用 EXIF 旋转标记，竖拍照片会横着发给模型，编码前按 EXIF 摆正。新增 LoyeaVision 诊断日志（路由决策/请求端点与模型/图片 base64 长度/解码失败原因），实机排障不再靠猜。
- **消息窗口化懒加载**：UI 只渲染锚点附近窗口（初始=最后发言前 8 条起），上滑/下滑动态补载与释放（流式期间不释放底部），长会话翻阅更流畅。
- **窗口化懒加载下标换算错位与滚动时序竞态**（真机反馈）：窗口 effect 将 LazyColumn 窗口内下标误当全局下标比较（载入定位漂移、列表骤缩预取越界闪退）；补偿滚动写在窗口 effect 自身协程内被 key 变更取消；锚定/回底滚动落在未提交的旧数据上被钳停。修复 = 下标换算全局化 + 切片改隔离副本 + 所有滚动等数据真正提交再执行（补偿滚动挪独立常驻 effect）+ 到底判定改全局语义。

（会话与会话书解耦：独占绑定的 owned 书若同时是全局生效书，仅解绑保留；非全局的独占书仍随会话删除）。

### Added (新增)
- **生图 b64_json 响应支持**：服务商仅返回 base64 编码图片时直接解码落盘展示（此前该形态会报错失败）；/draw 与对话式生图收敛为同一条生成链路。

### Changed (变更)
- 中文消息时间分段细化为凌晨 / 早上 / 上午 / 中午 / 下午 / 晚上 / 深夜七段（修正旧实现 23 点误标"下午 11:05"）；英语保持 AM/PM。
- 换书选择器与书库"设为全局生效"菜单过滤来源卡已删除的死卡书（数据层同时拒绝其生效）。
- 卡书摘要"已关 N 条"计数口径统一为合并卡内原生禁用与用户 override 后的实际禁用数。
- 换会话即重置内容锚点：经空会话中转返回时也会重新定位到最后一次用户消息处。
- 多轮工具响应期间不再把上滑回看的用户拽回底部（仅在用户仍位于底部时恢复自动跟随）。
- 工具卡标题人话化：内置工具映射为动作描述（双语），搜索/读网页带查询词，外部工具不再显示裸名。
- 工具轮耗尽后先发一次无工具收尾请求让 AI 基于已获信息把话说完（替代直接落系统占位文案）。
- 导入卡路径输出协议补齐时间标签正面说明与例外限定（与原生路径措辞对齐）。

## [0.7.2] - 2026-09-07

> v0.7.2 为一次大型版本重排，直接覆盖安装即可从 v0.5.5 升级（首次启动自动迁移数据，旧文件保留可回滚）。开发期内部里程碑 0.6.1 / 0.7.0 / 0.7.1 未单独发布，其变化一并计入本条目。

### Added (新增)
- **character-core 双模块架构**：新增纯 Kotlin 静态模块，承载角色卡编解码（V1/V2/V3 PNG/JSON）、角色文档模型、世界书匹配内核、有限正则引擎与提示词编译器；不依赖 Android，可在 JVM 上完整测试（147 项单元测试）。
- **统一角色文档**：原生人格与导入卡使用同一种 `CharacterDocument` 存储，角色名、人格、场景、系统指令、历史后指令、示例对话、备用开场白、内嵌世界书、正则脚本全部字段原样保留，未知扩展字段保真不丢。
- **数据迁移（rebuild_storage_v1）**：从旧版数据结构一次性迁移，staging 构建完成后原子切换；manifest 记录源文件快照（大小 + SHA-256）；迁移中断可重试、不产生半迁移状态；旧文件原样保留可回滚；冲突数据以「迁移冲突副本」条目呈现，两份都在。
- **世界书 2.0 统一书库**：全局书 / 会话书 / 角色卡内嵌书收敛为一个书库，设置页「世界书」唯一管理入口；单一生效书解析（会话绑定 > 随角色卡 > 全局生效 > 不使用）；条目级独立开关（含卡书条目）；书级配置覆盖（扫描深度、插入顺序、预算等）；标准 SillyTavern World Info JSON 往返导入导出。
- **卡书条目 override 编辑**：角色卡内嵌书条目内容可修改，改动存书库 override 层快照，角色卡原文件不动；「恢复原文」一键还原；删卡重导同卡 override 与开关自动恢复。
- **聊天页生效书面板**：右上角查看当前生效书（条目数 / 常驻数 / 来源），支持换书、跟随默认、跳转书库；换书下一轮生效，流式输出中的当轮不受影响；删除会话时其独占的会话书自动清理、共享书与卡书只解绑。
- **有限正则引擎（P5）**：角色卡 regex_scripts 按「输入提示词 / 输出提示词 / 输出显示」三类作用域执行，RE2/J 三阶段白名单映射；不支持的特性整条跳过并明确报告，有界执行防 ReDoS。
- **有限 HTML 状态面板**：角色卡要求的状态面板原生渲染（文字、结构、折叠交互）；无法渲染的内容折叠为「嵌入内容 · N 行 · 点击展开」降级展示。
- **联网搜索 API 全局配置**：模型管理页顶部统一配置一次，全模型共用；留空回落服务商自带联网或免 Key 检索。
- **模型列表云端同步**：一键 GET /models 拉取可用模型点选填入（覆盖 DeepSeek / OpenAI / Kimi / Qwen / Groq / Ollama / 兼容中转，Anthropic 走 x-api-key 适配）。
- **全新品牌图标**：双版 logo 原图入库、自适应启动图标（背景层米白 + 前景层居中安全区，圆形/圆角方遮罩完整）、通知栏白色剪影。
- **书库条目全文查看**：书详情页点条目正文弹出底部弹层查看全文、关键词与元信息，弹层内直达编辑。
- **AI 生图工具（generate_image）**：生图 API 配置好后，对话中直接说"画一张……"AI 即可调用生图工具创作并展示（此前模型不知道自己有生图能力，只有手动 `/draw` 指令可用）；与 `/draw` 指令共用同一条生成与本地保存链路。

### Changed (变更)
- **AI 生图展示优雅化**：AI 生成/工具生成的图片在气泡内以更大画幅完整展示（不再裁切），保留骨架加载与点击查看原图。
- **版本号升级至 v0.7.2**：Android 内部 `versionCode` 同步升级至 16，可覆盖安装 v0.5.5 及之后任意版本（签名一致）。
- **历史滑窗按总上下文预算选取**：替代固定「最近 20 条」裁剪，消除 20–160 条之间的摘要覆盖空档；工具调用与结果作为完整单元取舍。
- **提示词固定顺序合同与字节级稳定前缀**：提示词各区块按确定顺序编译，两条请求路径共用同一编译器；图谱记忆、底部世界书等易变上下文固化进不可变回合快照，扩大 DeepSeek 等前缀缓存命中。
- **时间感知改用户消息时间线**：每条用户消息在仅供模型的 payload 中携带发送时刻绝对时间，配合回合快照当前时刻提供完整时间流逝感；模型自身历史回复不带时间标签，从源头避免模型模仿元数据格式造成标签泄露。
- **删除全局「禁一切方括号」输出禁令**：角色卡自定义格式（状态面板等）优先；保留「不复述应用元数据、默认不输出时间标签（角色设定或用户明确要求时除外）」的窄规则。
- **设置页重组**：按「连接 / 模型 / 能力」三节组织；世界书管理入口唯一化为设置页「世界书」→ 统一书库（旧的双作用域世界书编辑器退役）。
- **角色按创建时间排序**；卡内世界书预览接入；Thinking 读秒锚定绝对时间戳，多轮请求不重置。

### Fixed (修复)
- **时间标签泄露根因**：旧实现为包括模型自身回复在内的每条历史消息添加时间标签，模型会模仿自己的输出格式导致标签出现在回复中（含流式半个标签）；现仅用户消息携带时间标签，配合窄禁令双保险。
- **面板与正文重叠**：修复 SelectionContainer 堆叠导致 HTML 面板压住消息正文的问题（面板可折叠）。
- **卡内世界书不可见**：修复导入卡的内嵌世界书在旧 UI 中无入口看似为空的问题（现进统一书库完整呈现）。
- **编辑框无界滚动与输入框重排跳动**：长文本编辑框限高滚动；输入框 padding 恒定，不再因折行变化导致光标跳动。
- **Thinking 计时漂移**：读秒改为锚定消息绝对时间戳。
- **lint 门禁清零**：59 个 lint error 全部处理——fragment 依赖显式声明（ActivityResult 契约要求）、手表蓝牙在权限守卫前读取设备名、录音缺运行时权限兜底守卫；0 error / 93 存量 warning，测试 147 项全绿。

### Architecture (架构影响)
- `app` + `character-core` 双模块：宿主能力（聊天、感知、记忆、语音、MCP）与角色内核（编解码、匹配、编译）分离；世界书匹配实现全应用唯一，UI 与网络层不含匹配细节。
- 世界书数据结构：书文档 `worldinfo/books/<bookId>.json`（含 disabledUids 与 entryOverrides override 层），一次性迁移 manifest 幂等短路；卡书为实时引用不落内容，卡文件只读。
- 提示词时间元数据：用户消息逐条 `[MESSAGE TIME]` + 回合快照 `System Time` 双来源；快照按用户回合冻结，缓存友好。
