...
- **窗口化懒加载下标换算错位**（真机反馈）：窗口 effect 将 LazyColumn 窗口内下标误当全局下标比较——长会话中「近窗口头」判定恒真把 windowStart 级联砍回 0（载入定位到第一条消息）；「远离底部」恒真把 windowEnd 骤缩，LazyColumn 预取器持旧下标撞收缩列表越界闪退。切片改隔离副本 + 锚定期间窗口化让位 + 头插/释放补偿滚动修复。
